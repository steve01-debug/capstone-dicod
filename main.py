...
def load_data():
    ...
    # Pencarian cerdas: cek di root atau cari di subfolder jika tidak ketemu
    target_path = None
    if os.path.exists(csv_file):
        ...
        for root, dirs, files in os.walk("."):
            if csv_file in files:
                target_path = os.path.join(root, csv_file)
                break
    ...
    if target_path and os.path.exists(target_path):
        ...
        # FALLBACK SAFETY: Jika file hilang, buat data tiruan agar aplikasi TIDAK CRASH
        np.random.seed(42)
        obat_list = [f"Medicine_{i}" for i in range(101, 115)]
        backup_data = []
        for obat in obat_list:
            stok = np.random.randint(0, 100)
            pred = np.random.randint(20, 90)
            backup_data.append({
                'medicine': obat,
                'stock_sekarang': stok,
...
# ============================================================
# PROSES DAN STANDARDISASI KOLOM
# ============================================================
...
# Membuat Status Alert jika belum tersedia otomatis
